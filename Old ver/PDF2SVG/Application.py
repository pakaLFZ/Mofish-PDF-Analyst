...
def Get_File_List():
    ...
    for root, dirs, files in os.walk(File_Storage, topdown=True):
        for name in files:
            File_Number += 1
            if re.match('.*(.pdf)', os.path.join(name)) is not None:  # 0620_m18_2_2_qp  0620_m15_qp_22 0439_w17_ms_21 0439_w17_gt  0625_s03_qp_1.pdf
                File_Match = 0
                if os.path.join(name)[8 : 11] == '_qp'and os.path.join(name)[11 : 13] == '_1':
                    File_Match = 1
                if os.path.join(name)[8 : 11] == '_1_'and os.path.join(name)[13 : 15] == 'qp':
                    File_Match = 1
                if os.path.join(name)[8 : 11] == '_qp'and os.path.join(name)[11 : 13] == '_2' and float(os.path.join(name)[6 : 8]) > 15:
                    File_Match = 1
                if os.path.join(name)[8 : 11] == '_2_'and os.path.join(name)[13 : 15] == 'qp':
                    File_Match = 1
                if os.path.join(name)[12 : 15] == '_2'and os.path.join(name)[10 : 12] == 'qp':
                    File_Match = 1
                if os.path.join(name)[12 : 15] == '_1'and os.path.join(name)[10 : 12] == 'qp':
                    File_Match = 1
