...
def Launcher():
	File_Position = 'E:/0Mofish/PDF/Table-17/All-MCQ-Raw-PDF/Product'
	...
	while Location <= len(File_List) - 1:
		if Location % 500 == 0:
			PRINT('-', 0)
		File_Content = Get_File_Content(File_List[Location], File_Position)
		Content = {}
	...
	#对答案
	PRINT('>', 1)
	Instructor = open('./Answer_Bank.txt', 'r').read()
	...
	Final_Data['chapterList'] = open('./Chapter_List.mofish', 'r').read()
...
def Get_Paper_List(File_List):
	...
	for Paper in Paper_List:
		...
		Data = Data +'{\n'
		Data = Data +'\t#ID:"' + str(Id_Count) + '",\n'
		Data = Data +'\t#Paper name:"' + str(Paper) + '",\n'
		Data = Data +'\t#Syllabus:"' + str(Syllabus) + '",\n'
		Data = Data +'\t#Year:"' + str(Year) + '",\n'
		Data = Data +'\t#Month:"' + str(Month) + '",\n'
		Data = Data +'\t#Paper_type:"' + str(Paper_type) + '",\n'
		Data = Data +'\t#Variant:"' + str(Variant) + '",\n'
		Data = Data +'}\n'
		Id_Count += 1
	return Data

def Get_File_Content(File_Name, File_Storage):
	File = open(File_Storage + '/' + File_Name, 'r', encoding='utf-8').read()
	Instructor = open('./Safety_Instructor.mofish', 'r').read()
	Location_1 = 0
	Content = ''
	while 1:
